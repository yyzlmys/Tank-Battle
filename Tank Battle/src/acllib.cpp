...
{ 
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == 1) buttonfunc();
		SetFocus(g_hWnd);
		break;

	case WM_CLOSE:
		res = MessageBox(hwnd, "莫西莫西，真的要关闭人家么？", "不要啊", MB_YESNO | MB_ICONQUESTION);
		if (res == IDYES)
		{
			MessageBox(hwnd, "跟我一起去二次元吧", "再见了", MB_OK);
			DestroyWindow(hwnd);
		}
		if (res == IDNO) MessageBox(hwnd, "我就知道你最好了！", "呐呐呐", MB_OK);
		break;
